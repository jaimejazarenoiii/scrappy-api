...
export const commonSchemas = {
  ...
  CompleteTripRequest: {
    ...
    properties: {
      note: { type: 'string', maxLength: 500 },
      endingOdometer: { type: 'number', minimum: 0 },
    },
  },
  CancelTripRequest: {
    type: 'object',
  ...
  CompanyAnalytics: {
    ...
    properties: {
      ...
      inboundAmount: { type: 'number', description: 'Money paid out buying scrap (INBOUND).' },
      outboundAmount: { type: 'number', description: 'Money received selling scrap (OUTBOUND).' },
  ...
  ActivityLog: {
    ...
    properties: {
      id: uuid,
      companyId: uuid,
      eventType: { type: 'string', example: 'Transaction' },
      module: { type: 'string', example: 'Transaction' },
      action: { type: 'string', example: 'Transaction paid' },
      description: { type: 'string', example: 'Transaction paid' },
      userId: uuid,
      employeeId: { ...uuid, nullable: true },
      resourceType: { type: 'string', nullable: true, example: 'Trip' },
