...
import { LEAVE_EXPORT_COLUMNS, mapLeaveExportRow } from '../export/report-export-columns.js';
...
export interface LeaveReportExportQuery extends ResolvedReportQuery {
  format: ReportExportFormat;
  disposition?: 'attachment' | 'inline';
}

export class ExportLeaveReportUseCase {
  constructor(
  ...
  async execute(
    ...
    logReportAccess('leave', 'export', ctx.filter, auth.userId, { format: query.format });
    ...
    const totalCount = await this.queryRepository.countLeaveReports(exportParams);
    return this.exportOrchestrator.export({
      ...
      mapRow: mapLeaveExportRow,
      fetchBatch: (skip, take) => this.queryRepository.batchLeaveReports(exportParams, skip, take),
    });
  }
}
