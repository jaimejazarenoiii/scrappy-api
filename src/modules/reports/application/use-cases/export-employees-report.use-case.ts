...
import { EMPLOYEE_EXPORT_COLUMNS, mapEmployeeExportRow } from '../export/report-export-columns.js';
...
export interface EmployeeReportExportQuery extends ResolvedReportQuery {
  format: ReportExportFormat;
  disposition?: 'attachment' | 'inline';
}

export class ExportEmployeeReportUseCase {
  constructor(
  ...
  async execute(
    ...
    query: EmployeeReportExportQuery,
    ...
    logReportAccess('employees', 'export', ctx.filter, auth.userId, { format: query.format });
    ...
    const totalCount = await this.queryRepository.countEmployeeReports(exportParams);
    return this.exportOrchestrator.export({
      ...
      mapRow: mapEmployeeExportRow,
      fetchBatch: (skip, take) =>
        this.queryRepository.batchEmployeeReports(exportParams, skip, take),
    });
  }
}
