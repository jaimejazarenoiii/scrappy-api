...
export interface TransactionReportExportQuery extends ResolvedReportQuery {
  format: ReportExportFormat;
  disposition?: 'attachment' | 'inline';
}

export class ExportTransactionReportUseCase {
  constructor(
  ...
  async execute(
    ...
    query: TransactionReportExportQuery,
    ...
    logReportAccess('transactions', 'export', ctx.filter, auth.userId, { format: query.format });
    ...
    const totalCount = await this.queryRepository.countTransactionReports(exportParams);
    return this.exportOrchestrator.export({
      ...
      mapRow: mapTransactionExportRow,
      fetchBatch: (skip, take) =>
        this.queryRepository.batchTransactionReports(exportParams, skip, take),
    });
  }
}
