...
import type { ExpenseReportRowDto } from '../dto/expense-report.response.js';
...
export class ListExpenseReportUseCase {
  constructor(
  ...
  async execute(
  ...
  ): Promise<ReportListResponseDto<ExpenseReportRowDto>> {
    ...
    logReportAccess('expenses', 'list', ctx.filter, auth.userId, { search: ctx.search ?? null });
    const result = await this.queryRepository.listExpenseReports({
      filter: ctx.filter,
