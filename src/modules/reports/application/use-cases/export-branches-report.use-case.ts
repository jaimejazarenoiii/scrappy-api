...
import { BRANCH_EXPORT_COLUMNS, mapBranchExportRow } from '../export/report-export-columns.js';
...
export interface BranchReportExportQuery extends ResolvedReportQuery {
...
export class ExportBranchReportUseCase {
  ...
  async execute(
    ...
    logReportAccess('branches', 'export', ctx.filter, auth.userId, { format: query.format });
    ...
    const totalCount = await this.queryRepository.countBranchReports(exportParams);
    return this.exportOrchestrator.export({
      ...
      mapRow: mapBranchExportRow,
      fetchBatch: (skip, take) => this.queryRepository.batchBranchReports(exportParams, skip, take),
    });
  }
}
