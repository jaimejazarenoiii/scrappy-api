...
import type { TripReportRowDto } from '../dto/trip-report.response.js';
...
export class ListTripReportUseCase {
  constructor(
  ...
  async execute(
  ...
  ): Promise<ReportListResponseDto<TripReportRowDto>> {
    ...
    logReportAccess('trips', 'list', ctx.filter, auth.userId, { search: ctx.search ?? null });
    const result = await this.queryRepository.listTripReports({
      filter: ctx.filter,
