...
import type { BranchReportRowDto } from '../dto/branch-report.response.js';
...
export class ListBranchReportUseCase {
  constructor(
  ...
  async execute(
  ...
  ): Promise<ReportListResponseDto<BranchReportRowDto>> {
    ...
    logReportAccess('branches', 'list', ctx.filter, auth.userId, { search: ctx.search ?? null });
    const result = await this.queryRepository.listBranchReports({
      filter: ctx.filter,
