...
export interface WarehouseReportExportQuery extends ResolvedReportQuery {
  format: ReportExportFormat;
  disposition?: 'attachment' | 'inline';
}

export class ExportWarehouseReportUseCase {
  constructor(
  ...
  async execute(
    ...
    query: WarehouseReportExportQuery,
    ...
    logReportAccess('warehouses', 'export', ctx.filter, auth.userId, { format: query.format });
    ...
    const totalCount = await this.queryRepository.countWarehouseReports(exportParams);
    return this.exportOrchestrator.export({
      ...
      mapRow: mapWarehouseExportRow,
      fetchBatch: (skip, take) =>
        this.queryRepository.batchWarehouseReports(exportParams, skip, take),
    });
  }
}
